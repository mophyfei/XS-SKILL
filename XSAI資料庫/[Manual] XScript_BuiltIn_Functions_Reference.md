...
title: XS完整手冊 - 內建函數
category: 內建函數
tags: [XS語法, 內建函數, BarFreq, PlotFill, TimeAdd, DateToJulian, Tan, GetBarBack, AvgList, GetTBMode, GetFirstBarDate, CoTangent, SetBarMode, MaxList2, Array_Copy, CallFunction, StrSplit, Symbol, UpperStr, Second, MaxList, GetBackBar, Cover, GetFieldPublishDate, DateToString, IsFirstCall, IsSessionFirstBar, SetBarFreq, GetBarOffset, StrToNum, ArcCosine, IsListedSymbol, MilliSecond, TimeDiff, DayOfMonth, Alert, Ceiling, Short, EncodeTime, UserID, FilledEntryTime, CurrentBar, IntPortion, Floor, File, SetAlign, Year, IsMarketPrice, GetSymbolGroup, Sell, StrLen, RightStr, FilledRecordIsRealtime, FracPortion, GetSymbolInfo, Array_Compare, Filled, FilledRecordPrice, LowerStr, FilledRecordQty, CurrentTimeMS, Print, IsSupportField, Market, Minute, BarInterval, Array_SetValRange, Array_GetMaxIndex, GroupSize, SetBarBack, WeekOfMonth, Sin, StrCompare, Neg, ArcTangent, Month, DataAlign, IsLastBar, FilledRecordDate, Array_Sort, DateAdd, TimeToString, CheckSymbolField, Array_GetType, FilledRecordTimeMS, Buy, DayOfWeek, ExpValue, CheckField, LeftStr, SetPlotLabel, StrTrim, GetQuote, InStr, Mod, Tangent, SetFirstBarDate, Factorial, Permutation, CurrentDate, MaxBarsBack, FilledRecordTime, Playsound, Plot, Combination, Array_Sum, GetFieldDate, NthMaxList, SetBackBar, SymbolExchange, SymbolName, ExecOffset, SetOutputName, Sign, Sine, GetFieldStartOffset, SetInputName, StringToTime, StrEndWith, DateValue, Log, JulianToDate, SetTBMode, FormatTime, ArcSine, Square, PlotK, SetPosition, DateDiff, Random, MidStr, StrStartWith, NthMinList, WeekOfYear, FilledRecordBS, BarAdjusted, DefaultBuyPrice, SquareRoot, FilledEntryDate, Cos, DefaultSellPrice, Power, Round, NoPlot, SymbolType, FilledAtBroker, Pos, OutputField, FilledRecordCount, StringToDate, RaiseRunTimeError, FilledEntryTimeMS, Cosine, GetField, IsSupportSymbolField, GetSymbolFieldDate, GetSymbolField, MinList2, AbsValue, IsSessionLastBar, MinList, GetTotalBar, FilledAvgPrice, FormatDate, Hour, Text, AddSpread, SetTotalBar, Array_Sort2d, EncodeDate, GetInfo, CancelAllOrders, NumToStr, SetRemoveOutlier, SumList, CurrentTime, GetSymbolFieldStartOffset, Array_SetMaxIndex, TimeValue, PlotLine, Position]
...
> 本文件收錄了 [內建函數] 下的所有子分類與函數說明，共 179 筆。
> 包含子分類：BarFreq, PlotFill, TimeAdd, DateToJulian, Tan, GetBarBack, AvgList, GetTBMode, GetFirstBarDate, CoTangent, SetBarMode, MaxList2, Array_Copy, CallFunction, StrSplit, Symbol, UpperStr, Second, MaxList, GetBackBar, Cover, GetFieldPublishDate, DateToString, IsFirstCall, IsSessionFirstBar, SetBarFreq, GetBarOffset, StrToNum, ArcCosine, IsListedSymbol, MilliSecond, TimeDiff, DayOfMonth, Alert, Ceiling, Short, EncodeTime, UserID, FilledEntryTime, CurrentBar, IntPortion, Floor, File, SetAlign, Year, IsMarketPrice, GetSymbolGroup, Sell, StrLen, RightStr, FilledRecordIsRealtime, FracPortion, GetSymbolInfo, Array_Compare, Filled, FilledRecordPrice, LowerStr, FilledRecordQty, CurrentTimeMS, Print, IsSupportField, Market, Minute, BarInterval, Array_SetValRange, Array_GetMaxIndex, GroupSize, SetBarBack, WeekOfMonth, Sin, StrCompare, Neg, ArcTangent, Month, DataAlign, IsLastBar, FilledRecordDate, Array_Sort, DateAdd, TimeToString, CheckSymbolField, Array_GetType, FilledRecordTimeMS, Buy, DayOfWeek, ExpValue, CheckField, LeftStr, SetPlotLabel, StrTrim, GetQuote, InStr, Mod, Tangent, SetFirstBarDate, Factorial, Permutation, CurrentDate, MaxBarsBack, FilledRecordTime, Playsound, Plot, Combination, Array_Sum, GetFieldDate, NthMaxList, SetBackBar, SymbolExchange, SymbolName, ExecOffset, SetOutputName, Sign, Sine, GetFieldStartOffset, SetInputName, StringToTime, StrEndWith, DateValue, Log, JulianToDate, SetTBMode, FormatTime, ArcSine, Square, PlotK, SetPosition, DateDiff, Random, MidStr, StrStartWith, NthMinList, WeekOfYear, FilledRecordBS, BarAdjusted, DefaultBuyPrice, SquareRoot, FilledEntryDate, Cos, DefaultSellPrice, Power, Round, NoPlot, SymbolType, FilledAtBroker, Pos, OutputField, FilledRecordCount, StringToDate, RaiseRunTimeError, FilledEntryTimeMS, Cosine, GetField, IsSupportSymbolField, GetSymbolFieldDate, GetSymbolField, MinList2, AbsValue, IsSessionLastBar, MinList, GetTotalBar, FilledAvgPrice, FormatDate, Hour, Text, AddSpread, SetTotalBar, Array_Sort2d, EncodeDate, GetInfo, CancelAllOrders, NumToStr, SetRemoveOutlier, SumList, CurrentTime, GetSymbolFieldStartOffset, Array_SetMaxIndex, TimeValue, PlotLine, Position
...
##### 說明
一般而言BarInterval函數會跟[BarFreq](api.aspx?a=barfreq&b=sys)一起搭配使用，用來判斷目前執行的分鐘頻率的分鐘間隔。
...
### CallFunction
#### CallFunction – （內建函數） <kbd>一般函數</kbd>
...
##### 說明
[Print指令](api.aspx?a=print&b=bif)如果沒有傳入指定檔案欄位時，預設會將所有的Print輸出到XQ安裝目錄底下的XS/Print子目錄內，輸出檔案的名稱為**策略名稱_商品代碼.log**，例如 **MyStrategy_2330.TW.log**。
...
請參考[File指令](api.aspx?a=file&b=bif)，以及[教學文章](https://xstrader.net/print%E8%AA%9E%E6%B3%95%E8%AA%BF%E6%95%B4/)。
...
詳細的語法說明可以參考 [GetFieldStartOffset](api.aspx?a=GetFieldStartOffset&b=bif)函數。
...
### GetSymbolGroup
#### GetSymbolGroup – （內建函數） <kbd>一般函數</kbd>
...
### IsSessionFirstBar
#### IsSessionFirstBar – （內建函數） <kbd>一般函數</kbd>
...
### IsSessionLastBar
#### IsSessionLastBar – （內建函數） <kbd>一般函數</kbd>
...
##### 說明
在指標腳本內我們會使用[Plot函數](api.aspx?a=plot&b=bif)來產生不同的數值序列，每一個數值序列有一個編號，從1到999。
...
OutputField指令內所設定的欄位標題也可以透過[SetOutputName函數](api.aspx?a=setoutputname&b=bif)來指定。

OutputField 也可以使用 [order](api.aspx?a=order) 來指定選股結果區的欄位數值上/下排序，請參考連結說明使用。
...
使用者也可以利用[File指令](api.aspx?a=file&b=bif)來指定輸出的目錄或是檔名，交易腳本必須用此法才能列印到檔案。
...
如果需要避開重覆Print在同一個檔案，可以運用[File指令](api.aspx?a=file&b=bif)搭配[StartTime]參數，讓每次執行的Print檔案可以分開不同目錄，檔案維護上比較方便。
...
上述是一個警示腳本，透過 [q_CurrentShareCapital](api.aspx?a=q_CurrentShareCapital) 欄位來判斷商品的股本是否小於10億，如果小於10億是的話則中斷執行。
...
![Input設定2](https://www.xq.com.tw/xstrader/wp-content/uploads/2015/09/Input_設定.png)
...
在XQ 5.60版之後，[OutputField指令](api.aspx?a=outputfield&b=bif)也增加了可以直接傳入欄位標題的功能。
...
### SetPlotLabel
#### SetPlotLabel – （內建函數） <kbd>一般函數</kbd>
...
這個函數可以用來指定這個繪圖序列的名稱，跟[Plot函數](api.aspx?a=plot&b=bif)的第二個參數是類似的。
...
由於天期是透過[Input語法](api.aspx?a=input)傳入的，數值可以動態被修改，沒有辦法寫成一個固定的字串，所以我們使用SetPlotLabel，搭配[Text函數](api.aspx?a=text&b=bif)以及[NumToStr函數](api.aspx?a=numtostr&b=bif)來組出天期的字串。
...
呼叫時需要傳入兩個參數：第一個參數是基礎價格，第二個參數是加減檔數，如果要加檔數的話請傳入一個大於0的數值，如果要減檔數的話請傳入一個小於0的數值。<u>如果商品有漲跌停限制的話，計算後的數值不會超過漲跌停限制。</u>
...
### CancelAllOrders
#### CancelAllOrders – （內建函數） <kbd>交易函數</kbd>
...
### DefaultBuyPrice
#### DefaultBuyPrice – （內建函數） <kbd>交易函數</kbd>
...
如果策略指定預設買進價格為市價的話，DefaultBuyPrice會回傳商品的漲停價(如果商品沒有漲停跌停價的話，則回傳洗價K棒的收盤價)。如果指定的是觸發價 +/- 檔數的話，則依照回傳洗價K棒的收盤價加減檔數後的價格。與[AddSpread](api.aspx?a=AddSpread&b=bif)一樣，加減檔數後的數值不會超過商品的漲跌停限制。
...
### DefaultSellPrice
#### DefaultSellPrice – （內建函數） <kbd>交易函數</kbd>
...
如果策略指定預設賣出價格為市價的話，DefaultSellPrice會回傳商品的跌停價(如果商品沒有漲停跌停價的話，則回傳洗價K棒的收盤價)。如果指定的是觸發價 +/- 檔數的話，則回傳依照洗價K棒的收盤價加減檔數後的價格。與[AddSpread](api.aspx?a=AddSpread&b=bif)一樣，加減檔數後的數值不會超過商品的漲跌停限制。
...
如同Position，Filled也是<u>一個數值序列</u>。Filled或是Filled[0]代表的是目前這一根bar的成交部位數值。Filled[1]則是上一根bar執行時的成交部位數值。如果開啟逐筆洗價的話，每一根bar內可能會執行多次，Filled的數值則會在每一次洗價之前就會依照當時的成交狀態更新。
...
在這個範例內腳本透過判斷Filled是否是1來決定是否已經成交。一旦成交之後，則透過比對目前價格與未平倉成本[FilledAvgPrice](api.aspx?a=FilledAvgPrice&b=bif)是否達到停損停利的範圍來決定是否要進行平倉。
...
### FilledAtBroker
#### FilledAtBroker – （內建函數） <kbd>交易函數</kbd>
...
### FilledEntryDate
#### FilledEntryDate – （內建函數） <kbd>交易函數</kbd>
...
### FilledEntryTime
#### FilledEntryTime – （內建函數） <kbd>交易函數</kbd>
...
### FilledEntryTimeMS
#### FilledEntryTimeMS – （內建函數） <kbd>交易函數</kbd>
...
### FilledRecordBS
#### FilledRecordBS – （內建函數） <kbd>交易函數</kbd>
...
### FilledRecordCount
#### FilledRecordCount – （內建函數） <kbd>交易函數</kbd>
...
##### 說明
腳本執行過程內，系統會紀錄每一筆成交紀錄，依照成交時間排序，FilledRecordCount則代表這些成交紀錄的筆數。透過FilledRecordCount，以及其他FilledRecord開頭的函數，包含[FilledRecordDate](api.aspx?a=FilledRecordDate&b=bif)，[FilledRecordTime](api.aspx?a=FilledRecordTime&b=bif)，[FilledRecordPrice](api.aspx?a=FilledRecordPrice&b=bif)，[FilledRecordBS](api.aspx?a=FilledRecordBS&b=bif)，[FilledRecordQty](api.aspx?a=FilledRecordQty&b=bif)等，來取得完整的成交紀錄資訊。
...
### FilledRecordDate
#### FilledRecordDate – （內建函數） <kbd>交易函數</kbd>
...
### FilledRecordPrice
#### FilledRecordPrice – （內建函數） <kbd>交易函數</kbd>
...
不管這一筆成交是買進或是賣出，FilledRecordPrice回傳的數值都是大於0的數字。如果需要判斷是買進或是賣出，請使用[FilledRecordBS](api.aspx?a=FilledRecordBS&b=bif)函數。
...
### FilledRecordQty
#### FilledRecordQty – （內建函數） <kbd>交易函數</kbd>
...
不管這一筆成交是買進或是賣出，FilledRecordQty回傳的數值都是大於0的數字。如果需要判斷是買進或是賣出，請使用[FilledRecordBS](api.aspx?a=FilledRecordBS&b=bif)函數。
...
### FilledRecordTime
#### FilledRecordTime – （內建函數） <kbd>交易函數</kbd>
...
### FilledRecordTimeMS
#### FilledRecordTimeMS – （內建函數） <kbd>交易函數</kbd>
...
### IsListedSymbol
#### IsListedSymbol – （內建函數） <kbd>交易函數</kbd>
...
![市價買](https://www.xq.com.tw/wp-content/uploads/2020/10/市價買.jpg)
...
![市價賣](https://www.xq.com.tw/wp-content/uploads/2020/10/市價賣.jpg)
...
Position跟Close、Value1等內建的變數一樣，都是一個數值序列。Position或是Position[0]代表的是目前這一根bar的數值。Position[1]則是上一根bar執行時的部位數值。要特別注意的是如果在一根bar內透過逐筆洗價執行了多次交易指令的話，Position的數值會在每一次洗價之後就會異動，也就是說Position是一個[intrabarpersist](api.aspx?a=intrabarpersist)的變數。

請參考SetPosition語法內關於[Position異動的時機點](api.aspx?a=SetPosition&b=bif#PositionChangeTimingAnchor)的相關說明。
...
把腳本的部位變成1，委託價格使用策略預設的買賣價格。如果原先的Position是0的話，這個指令會買進1張，如果原先的Position是2的話，這個指令會賣出1張。如果原先的Position是1的話，這個指令會檢視目前委託的執行情形，可能會送出改價的委託(如果原先委託尚未成交，且這一次的委託價格跟上一筆委託單的委託價格不一樣的話)，或是不做任何動作。詳細的執行邏輯請參考底下「[交易指令的執行方式](api.aspx?a=SetPosition&b=bif#TradeOrderAnchor)」的說明。
...
[Position](api.aspx?a=Position&b=bif)是一個內建的欄位，腳本可以透過這個欄位取得目前的部位數值。
[AddSpread](api.aspx?a=AddSpread&b=bif)函數可以用來計算加減檔數後的價格。

除了SetPosition語法可以改變腳本部位之外，系統同時還提供[Buy](api.aspx?a=Buy&b=bif)、[Sell](api.aspx?a=Sell&b=bif)、[Short](api.aspx?a=Short&b=bif)、[Cover](api.aspx?a=Cover&b=bif)這幾種語法，請參考相關說明。
...
可以透過label這個參數, 傳入這次SetPosition的標記名稱。請參考底下[交易指令的標記名稱](api.aspx?a=SetPosition&b=bif#TradeLabelAnchor)的說明。

<br><br>
##### Position跟Filled的關係
[Position](api.aspx?a=Position&b=bif)欄位代表腳本的預期部位，而[Filled](api.aspx?a=Filled&b=bif)欄位則是到目前為止的成交部位。簡單的來說，當腳本呼叫SetPosition傳入預期部位之後，系統就會依照SetPosition的參數來傳送委託，當收到成交時就會更新Filled欄位，如果Position跟Filled兩個欄位的數值一模一樣時，代表的是目前腳本的部位狀態與使用者的預期是一致的。

關於Position跟Filled，還有兩個地方要務必注意：

第一：一個自動交易策略可能會執行多個商品，每個商品的Position/Filled都是互相獨立的。

第二：Position/Filled的數值是一個<u>相對的數值</u>，與使用者所指定的交易帳號內這個商品的庫存部位未必是一樣的。例如我的交易帳號內可能有0050的庫存2張，可是XS自動交易策略提供多種[設定策略部位的方式](http://www.xq.com.tw/lesson/xsat/xsat_overview/#stock)，可以把Position設定從0開始，或是從2開始，甚至也可以設定成從1開始。
...
![help_自動交易策略_05](https://www.xq.com.tw/wp-content/uploads/2020/10/help_自動交易策略_05-1.png)
...
系統也支援檔位計算的函數([AddSpread](api.aspx?a=AddSpread&b=bif))，例如：
...
由於不同的商品有每次交易最大數量的限制(例如台股每一筆委託單只能送出499張)，如果SetPosition傳入的目標部位與先前的部位的差異超過這個限制的話，<u>為了安全起見，執行時系統會回傳錯誤(超過單筆委託限制)，同時停止這個商品的執行。</u>

<br><br>
<a name="TradeLabelAnchor"></a>
##### 交易指令的標記名稱
...
![標記名稱_V2](https://www.xq.com.tw/wp-content/uploads/2020/11/20201225_標記名稱_V2.png)
    
<br><br>
<a name="PositionChangeTimingAnchor"></a>
##### Position異動的時機點
...
<u>這兩個變數的數值在執行這一次腳本的過程內都不會改變。</u>當這次腳本執行結束之後，系統知道這次執行時呼叫了SetPosition(1)，所以在執行腳本完成後，才會把Position的數值改成1，同時準備送出委託單。

如果接下來商品收到了價格異動，再次執行腳本的話，此時Position的數值就會是1，而Filled的數值則依照當時是否已經收到了成交來決定是0還是1。

__規則#1: 系統會在執行腳本(洗價)前決定Position跟Filled的數值。__

__規則#2: 這兩個數值在執行腳本(洗價)的過程之間都不會改變。不會因為腳本呼叫了SetPosition之後就馬上改變Position的數值，也不會因為在這一次執行腳本(洗價)期間收到了成交而馬上改變Filled的數值。__
...
目前XS自動交易的執行方式是<u>只執行第一個交易指令</u>，忽略之後的交易指令。以上面腳本範例而言，如果condition1是false，condition2是true，condition3也是true的話，那麼當次洗價的第一個交易指令是SetPosition(2)，所以系統會執行這一個，至於之後呼叫的SetPosition(3)則予以忽略。
  
<br><br>
<a name="TradeOrderAnchor"></a>
##### 交易指令的執行方式
當系統收到了SetPosition指令時，系統會依照以下狀態來判斷如何執行這一筆交易指令：
- 目前的Position(執行這一個交易指令之前的Position數值)，
- 目前的成交部位(在腳本內稱之為[Filled](api.aspx?a=Filled&b=bif)，代表這個商品執行到目前為止的淨成交張數)，
- 是否先前的交易指令還有尚未完全成交的委託單

__範例#1: SetPosition(1)，買進1張後成交，SetPosition(0)，賣出1張__
...
__範例#2: SetPosition(1)，買進1張後未成交，SetPosition(0)，執行刪單__
...
__範例#3: SetPosition(1)，買進1張後成交，SetPosition(2)，再買進1張__
...
__範例#4: SetPosition(1)，買進1張後沒有成交，SetPosition(2)，刪除先前委託改成買進2張__
...
__範例#5: SetPosition(1)，買進1張後沒有成交，再次收到SetPosition(1)，刪除先前委託，改送不同價格的委託__
...
請參考 [RightStr函數](api.aspx?a=rightstr&b=bif) 以及 [MidStr函數](api.aspx?a=midstr&b=bif)。
...
請參考[UpperStr函數](api.aspx?a=upperstr&b=bif)
...
請參考 [LeftStr函數](api.aspx?a=leftstr&b=bif) 以及 [RightStr函數](api.aspx?a=rightstr&b=bif)。
...
請參考[StrToNum函數](api.aspx?a=strtonum&b=bif)。
...
### StrStartWith
#### StrStartWith – （內建函數） <kbd>字串函數</kbd>
...
請參考[NumToStr函數](api.aspx?a=numtostr&b=bif)。
...
這個函數可以搭配[Print函數](api.aspx?a=print&b=bif)來控制印出來的結果。
...
請參考[LowerStr函數](api.aspx?a=lowerstr&b=bif)
...
請注意: 如果要計算序列型的數值的平均值的話，則可以使用[Average函數](api.aspx?a=average&b=sys)。
...
請參考 [Permutation函數](api.aspx?a=permutation&b=bif)
...
計算三角函數的餘絃函數。與[Cos函數](api.aspx?a=cos&b=bif)相同。
...
請參考[Log函數](api.aspx?a=log&b=bif)。
...
請參考 [Ceiling函數](api.aspx?a=ceiling&b=bif) 以及 [Round函數](api.aspx?a=round&b=bif)
...
請參考 [IntPortion函數](api.aspx?a=intportion&b=bif)。
...
請參考 [FracPortion函數](api.aspx?a=fracportion&b=bif)。
...
請參考[ExpValue函數](api.aspx?a=expvalue&b=bif)。
...
腳本內同時使用到[MinList函數](api.aspx?a=minlist&b=bif)，這個函數的用法類似MaxList，傳入多個數值後回傳這些數值的最小值。
...
腳本內同時使用到[MaxList函數](api.aspx?a=maxlist&b=bif)，這個函數的用法類似MinList，傳入多個數值後回傳這些數值的最大值。
...
請參考[Pos函數](api.aspx?a=pos&b=bif)。
...
當排名位置為1時，NthMaxList函數等同於[MaxList函數](api.aspx?a=maxlist&b=bif)。當排名位置為最後一名時，NthMaxList函數等同於[MinList函數](api.aspx?a=minlist&b=bif)。
...
請參考 [NthMaxList函數](api.aspx?a=nthmaxlist&b=bif)。
...
請參考 [Combination函數](api.aspx?a=combination&b=bif)
...
這個函數的結果與[AbsValue函數](api.aspx?a=absvalue&b=bif)相同。
...
請參考 [Ceiling函數](api.aspx?a=ceiling&b=bif) 以及 [Floor函數](api.aspx?a=floor&b=bif)
...
請參考 [SquareRoot函數](api.aspx?a=squareroot&b=bif)。
...
請參考 [Square函數](api.aspx?a=square&b=bif)。
...
請注意: 如果要計算序列型的數值的加總值的話，則可以使用[Summation函數](api.aspx?a=summation&b=sys)。
...
計算三角函數的正切函數。與[Tan函數](api.aspx?a=tan&b=bif)相同。
...
### DateToString
#### DateToString – （內建函數） <kbd>日期函數</kbd>
...
請參考[StringToDate函數](api.aspx?a=stringtodate&b=bif)。
...
這個函數可以看成是[Year函數](api.aspx?a=year&b=bif)，[Month函數](api.aspx?a=month&b=bif)，[DayOfMonth函數](api.aspx?a=dayofmonth&b=bif)，[DayOfWeek函數](api.aspx?a=dayofweek&b=bif)，[WeekOfMonth函數](api.aspx?a=weekofmonth&b=bif)，以及[WeekOfYear函數](api.aspx?a=weekofyear&b=bif)等函數的綜合體。
...
日期相關的函數請參考[Year函數](api.aspx?a=year&b=bif)，[Month函數](api.aspx?a=month&b=bif)，[DayOfMonth函數](api.aspx?a=dayofmonth&b=bif)，[DayOfWeek函數](api.aspx?a=dayofweek&b=bif)，[WeekOfMonth函數](api.aspx?a=weekofmonth&b=bif)，以及[WeekOfYear函數](api.aspx?a=weekofyear&b=bif)。
...
### JulianToDate
#### JulianToDate – （內建函數） <kbd>日期函數</kbd>
...
請參考[DateToJulian函數](api.aspx?a=DateToJulian&b=bif)的說明。
...
### StringToDate
#### StringToDate – （內建函數） <kbd>日期函數</kbd>
...
請參考[DateToString函數](api.aspx?a=datetostring&b=bif)。
...
### CurrentTimeMS
#### CurrentTimeMS – （內建函數） <kbd>時間函數</kbd>
...
上述範例則是傳入[CurrentTime](api.aspx?a=currentime&b=bif)，也就是目前電腦的時間，格視為 HHMMSS。

---

也可以傳入含有毫秒的時間數值 [CurrentTimeMS](api.aspx?a=currentimems&b=bif) 回傳格式為 HHMMSS.fff
...
上述範例是傳入[CurrentTimeMS](api.aspx?a=currentimems&b=bif)，也是目前電腦的時間，不過是含有毫秒的電腦的時間，格式為 HHMMSS.fff。

時間相關的的函數請參考[Hour函數](api.aspx?a=hour&b=bif)，[Minute函數](api.aspx?a=minute&b=bif)，以及[Second函數](api.aspx?a=second&b=bif)。
...
時間相關的的函數請參考[Hour函數](api.aspx?a=hour&b=bif)，[Minute函數](api.aspx?a=minute&b=bif)，[Second函數](api.aspx?a=second&b=bif)，以及[MilliSecond](api.aspx?a=millisecond&b=bif)
...
含有毫秒的時間數值，通常是透過CurrentTimeMS，[FilledRecordTimeMS](api.aspx?a=FilledRecordTimeMS&b=bif)，或是其他時間相關函數所產生的時間數值。
...
### StringToTime
#### StringToTime – （內建函數） <kbd>時間函數</kbd>
...
請參考[TimeToString函數](api.aspx?a=timetostring&b=bif)。
...
通常是透過CurrentTimeMS，[FilledRecordTimeMS](api.aspx?a=FilledRecordTimeMS&b=bif)，或是其他含有毫秒的時間相關函數所產生的時間數值。
...
### TimeToString
#### TimeToString – （內建函數） <kbd>時間函數</kbd>
...
請參考[StringToTime函數](api.aspx?a=stringtotime&b=bif)。
...
含有毫秒的時間數值通常是透過CurrentTimeMS，[FilledRecordTimeMS](api.aspx?a=FilledRecordTimeMS&b=bif)，或是其他時間相關函數所產生的時間數值。
...
這個函數可以看成是 [Hour](api.aspx?a=hour&b=bif), [Minute](api.aspx?a=minute&b=bif), [Second](api.aspx?a=second&b=bif), 以及 [MilliSecond](api.aspx?a=MilliSecond&b=bif) 還有的綜合體。
...
### CheckSymbolField
#### CheckSymbolField – （內建函數） <kbd>欄位函數</kbd>
...
### GetFieldPublishDate
#### GetFieldPublishDate – （內建函數） <kbd>欄位函數</kbd>
...
##### 說明
在XS腳本內除了可以讀取資料欄位之外([GetField函數](api.aspx?a=getfield&b=bif))，也可以讀取即時的報價欄位。
...
上面的這個警示腳本內判斷如果當日開在最高([q\_DailyHigh](api.aspx?a=q_dailyhigh) = [q\_DailyOpen](api.aspx?a=q_dailyopen))，而且是跳空開出 (q\_DailyOpen > [q_RefPrice](api.aspx?a=q_refprice) * (1 + 跳空比例/100)，而目前的價格 [q\_Last](api.aspx?a=q_last)已經回落一定範圍的話則觸發警示。在這裡q\_DailyHigh為當日的最高價, q\_DailyOpen為當日的開盤價, q\_RefPrice為當日的參考價, q\_Last為當日的最新價格，這些都是常用的報價欄位。
...
![插入標的商品欄位](https://www.xq.com.tw/wp-content/uploads/2021/07/20210721_插入標的商品.png)
...
詳細的語法說明可以參考 [GetField](api.aspx?a=getfield&b=bif)函數。
...
詳細的語法說明可以參考 [GetFieldDate](api.aspx?a=GetFieldDate&b=bif)函數。
...
### GetSymbolInfo
#### GetSymbolInfo – （內建函數） <kbd>欄位函數</kbd>
...
### IsSupportSymbolField
#### IsSupportSymbolField – （內建函數） <kbd>欄位函數</kbd>
...
如果有多個商品同時執行同一個腳本時(例如策略雷達)，可以利用這個函數來判斷目前執行的商品而做不同的處理，也可以利用這個函數搭配[Print函數](api.aspx?a=print&b=bif)來輸出目前執行的商品代碼。
...
請參考[SymbolName函數](api.aspx?a=symbolname&b=bif)。
...
### Array_Compare
#### Array_Compare – （內建函數） <kbd>陣列函數</kbd>
...
### Array_GetType
#### Array_GetType – （內建函數） <kbd>陣列函數</kbd>
...
### Array_Sort2d
#### Array_Sort2d – （內建函數） <kbd>陣列函數</kbd>
